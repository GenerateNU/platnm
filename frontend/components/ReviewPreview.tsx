	...
	useFocusEffect(
		useCallback(() => {
			const fetchReview = async () => {
				try {
					const response = await axios.get(`${BASE_URL}/reviews/${preview.review_id}`);
					if (response.data) {
	...
	useFocusEffect(
		useCallback(() => {
			const fetchVote = async () => {
				try {
					const response = await axios.get(`${BASE_URL}/reviews/vote/${user_Id}/${preview.review_id}`);
					if (response.data) {
	...
	return (
		<View style={styles.card}>
			<View style={styles.vinyl}>
				...
				{preview.media_cover && (
					<Image source={{ uri: preview.media_cover }} style={styles.mediaCover} resizeMode='cover' />
				)}
			</View>
			...
			<View style={styles.actionsContainer}>
				<View style={styles.voteContainer}>
					<TouchableOpacity onPress={() => handleUpvotePress()} style={styles.voteButton}>
						<Upvote
							width={24}
							height={24}
					...
					<TouchableOpacity onPress={() => handleDownvotePress()} style={styles.voteButton}>
						<Downvote
							width={24}
							height={24}
							fill={currentVote && !currentVoteValue ? '#F28037' : '#555'}

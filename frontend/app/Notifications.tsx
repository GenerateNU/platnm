			...
			try {
				...
				response.data.forEach(async (notification: any) => {
					...
					if (notification.thumbnail.indexOf('https://example.com/') > -1) {
						notification.thumbnail = 'https://avatars.githubusercontent.com/u/59008111?s=40&v=4';
					}
					// check if older than 7 days
					const SEVEN_DAYS = 7 * 24 * 60 * 60 * 1000;
					if (new Date(notification.created_at) < new Date(Date.now() - SEVEN_DAYS)) {
						oldNotifications.push(notification);
					} else {
						newNotifications.push(notification);
					}
					// check if read
					notification.read = (await AsyncStorage.getItem('notification:' + notification.id)) === 'true';
					if (!notification.read) {
						await AsyncStorage.setItem('notification:' + notification.id, 'true');
					}
				});
